...
int argmax(const Eigen::RowVectorXf& vec) {
    Eigen::Index max_index;
    vec.maxCoeff(&max_index);
    return static_cast<int>(max_index);
}

float calculateAccuracy(const Eigen::MatrixXf& softmax_output, const Eigen::VectorXf& y_true) {
    int correct = 0;
    ...
    return static_cast<float>(correct) / softmax_output.rows();
}

// Overload for one-hot encoded targets
float calculateAccuracy(const Eigen::MatrixXf& softmax_output, const Eigen::MatrixXf& y_true) {
    int correct = 0;
    for (int i = 0; i < softmax_output.rows(); ++i) {
        int pred = argmax(softmax_output.row(i));
