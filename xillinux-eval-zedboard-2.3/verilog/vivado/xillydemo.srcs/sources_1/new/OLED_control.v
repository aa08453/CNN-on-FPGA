...
reg [6:0] clearColumn; // Counter for columns (0–127)
reg [2:0] clearPage;   // Counter for pages (0–7)
...
begin
    ...
    begin
        ...
        spiData <= 8'd0;
        spiLoadData <= 1'b0; 
        currPage <= 2; // starting from second page 
        sendDone <= 0;
        columnAddr <= 0;
    end
    else
    begin
        case(state)
            ...
            CLEAR_SCREEN: begin
                oled_dc_n <= 1'b1; // Data mode
                spiData <= 8'h00; // Send zero byte
                spiLoadData <= 1'b1;
                if (spiDone) begin
                    spiLoadData <= 1'b0;
                    if (clearColumn < 127) begin
                        clearColumn <= clearColumn + 1;
                    end else if (clearPage < 7) begin
                        clearColumn <= 0;
                        ...
                        clearPage <= 0;
                        state <= WAIT_SPI;
                        nextState <= PAGE_ADDR; // Move to the next initialization step
                    end
                end
            end
