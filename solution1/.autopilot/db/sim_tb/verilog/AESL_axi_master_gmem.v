...
module AESL_axi_master_gmem (
    ...
    TRAN_gmem_BUSER,
    TRAN_gmem_input_V,
    TRAN_gmem_outputConv_V,
    TRAN_gmem_weight_V,
    TRAN_gmem_bias_V,
    TRAN_gmem_outputPool_V,
    TRAN_gmem_weight2_V,
    TRAN_gmem_bias2_V,
    TRAN_gmem_outputConv2_V,
    TRAN_gmem_outputPool2_V,
    TRAN_gmem_outputDense_V,
    TRAN_gmem_fcWeight_V,
    TRAN_gmem_fcBias_V,
    ready,
    done
    );
...
//------------------------Parameter----------------------
`define TV_IN_gmem "./c.top.autotvin_gmem.dat"
`define TV_OUT_gmem "./rtl.top.autotvout_gmem.dat"
...
parameter gmem_mem_depth = 32'd 21652;
...
output [32 - 1 : 0] TRAN_gmem_input_V;
output [32 - 1 : 0] TRAN_gmem_outputConv_V;
output [32 - 1 : 0] TRAN_gmem_weight_V;
output [32 - 1 : 0] TRAN_gmem_bias_V;
output [32 - 1 : 0] TRAN_gmem_outputPool_V;
output [32 - 1 : 0] TRAN_gmem_weight2_V;
output [32 - 1 : 0] TRAN_gmem_bias2_V;
output [32 - 1 : 0] TRAN_gmem_outputConv2_V;
output [32 - 1 : 0] TRAN_gmem_outputPool2_V;
output [32 - 1 : 0] TRAN_gmem_outputDense_V;
output [32 - 1 : 0] TRAN_gmem_fcWeight_V;
output [32 - 1 : 0] TRAN_gmem_fcBias_V;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [32 - 1 : 0] input_V = 0;
reg [32 - 1 : 0] outputConv_V = 0;
reg [32 - 1 : 0] weight_V = 0;
reg [32 - 1 : 0] bias_V = 0;
reg [32 - 1 : 0] outputPool_V = 0;
reg [32 - 1 : 0] weight2_V = 0;
reg [32 - 1 : 0] bias2_V = 0;
reg [32 - 1 : 0] outputConv2_V = 0;
reg [32 - 1 : 0] outputPool2_V = 0;
reg [32 - 1 : 0] outputDense_V = 0;
reg [32 - 1 : 0] fcWeight_V = 0;
reg [32 - 1 : 0] fcBias_V = 0;
...
assign    TRAN_gmem_input_V = input_V;
assign    TRAN_gmem_outputConv_V = outputConv_V;
assign    TRAN_gmem_weight_V = weight_V;
assign    TRAN_gmem_bias_V = bias_V;
assign    TRAN_gmem_outputPool_V = outputPool_V;
assign    TRAN_gmem_weight2_V = weight2_V;
assign    TRAN_gmem_bias2_V = bias2_V;
assign    TRAN_gmem_outputConv2_V = outputConv2_V;
assign    TRAN_gmem_outputPool2_V = outputPool2_V;
assign    TRAN_gmem_outputDense_V = outputDense_V;
assign    TRAN_gmem_fcWeight_V = fcWeight_V;
assign    TRAN_gmem_fcBias_V = fcBias_V;
...
initial begin : initialize_offset
  ...
  outputConv_V <= 784 * DATA_byte_num;
  weight_V <= 7056 * DATA_byte_num;
  bias_V <= 7128 * DATA_byte_num;
  outputPool_V <= 7136 * DATA_byte_num;
  weight2_V <= 8704 * DATA_byte_num;
  bias2_V <= 9856 * DATA_byte_num;
  outputConv2_V <= 9872 * DATA_byte_num;
  outputPool2_V <= 13008 * DATA_byte_num;
  outputDense_V <= 13792 * DATA_byte_num;
  fcWeight_V <= 13802 * DATA_byte_num;
  fcBias_V <= 21642 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("D:/CNN2.0/hls/cnn/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("D:/CNN2.0/hls/cnn/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 9872; i < 13008 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      for(i = 13008; i < 13792 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      for(i = 13792; i < 13802 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      for(i = 13802; i < 21642 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      for(i = 21642; i < 21652 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
