...
always @(posedge ACLK) begin
    if (ARESET)
        int_outputDense[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUTDENSE_DATA_0)
            int_outputDense[31:0] <= (WDATA[31:0] & wmask) | (int_outputDense[31:0] & ~wmask);
    end
end

// int_outputDense[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_outputDense[63:32] <= 0;
    else if (ACLK_EN) begin
