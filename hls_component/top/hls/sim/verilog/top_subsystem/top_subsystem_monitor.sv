...
`ifndef TOP_SUBSYSTEM_MONITOR_SV
`define TOP_SUBSYSTEM_MONITOR_SV
...
class top_subsystem_monitor extends uvm_component;

    top_reference_model refm;
    top_scoreboard scbd;

    `uvm_component_utils_begin(top_subsystem_monitor)
    `uvm_component_utils_end

    uvm_analysis_imp_axi_wtr_gmem0#(axi_pkg::axi_transfer, top_subsystem_monitor) gmem0_wtr_imp;
    uvm_analysis_imp_axi_rtr_gmem0#(axi_pkg::axi_transfer, top_subsystem_monitor) gmem0_rtr_imp;
    uvm_analysis_imp_axi_wtr_gmem1#(axi_pkg::axi_transfer, top_subsystem_monitor) gmem1_wtr_imp;
    uvm_analysis_imp_axi_rtr_gmem1#(axi_pkg::axi_transfer, top_subsystem_monitor) gmem1_rtr_imp;
    uvm_analysis_imp_axi_wtr_control#(axi_pkg::axi_transfer, top_subsystem_monitor) control_wtr_imp;
    uvm_analysis_imp_axi_rtr_control#(axi_pkg::axi_transfer, top_subsystem_monitor) control_rtr_imp;

    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        if (!uvm_config_db#(top_reference_model)::get(this, "", "refm", refm))
            `uvm_fatal(this.get_full_name(), "No refm from high level")
        `uvm_info(this.get_full_name(), "get reference model by uvm_config_db", UVM_MEDIUM)
        scbd = top_scoreboard::type_id::create("scbd", this);
    endfunction
